...
filedir_w="C:\Users\J B\tubcloud\Module\Masterarbeit\Testen\Daten_bearbeitet\2019 04 16 19 58 19\2019 04 16 19 58 19_wind_v2.txt";; //Nur _v2.txt Daten
filedir_c="C:\Users\J B\tubcloud\Module\Masterarbeit\Testen\Daten_bearbeitet\2019 04 16 19 58 19\2019 04 16 19 58 19.txt";
...
////////////////////// Lücke über 5 sec in Control Daten füllen //////
...
Data_c_work=Data_c;
    ...
    Datenum_c_work(length(Datenum_c_work))+60/(24*60*60); //Hier Sekunden eintragen 
...
//    gap(i)=(c_dt_gap(i)+1)-(c_dt_gap(i));
    Datenum_c_gapfill=Datenum_c_work(c_dt_gap(i)):1/(24*60*60):Datenum_c_work(c_dt_gap(i)+1); //1 Sek. Daten erstellen
    ...
    Datenum_c_gapfill=datevec(Datenum_c_gapfill(1:length_gap));
    
    
        Date_corr=find(Datenum_c_gapfill(:,6)>59.5);
        Datenum_c_gapfill(Date_corr,6)=0;
        Datenum_c_gapfill(Date_corr,5)=Datenum_c_gapfill(Date_corr,5)+1;
        
        Date_corr=find(Datenum_c_gapfill(:,5)>59.5)
        Datenum_c_gapfill(Date_corr,5)=0;
        Datenum_c_gapfill(Date_corr,4)=Datenum_c_gapfill(Date_corr,4)+1;

    Data_c_work(c_dt_gap(i)+1:c_dt_gap(i)+length_gap,:)=%nan;
    Data_c_work(c_dt_gap(i)+1:c_dt_gap(i)+length_gap,1:6)=Datenum_c_gapfill;
end
...
filedir_neu=strcat([filedir_neu, '_all_v3_1sec.txt']);
